import { API_BUDGET_REQUEST, API_REQUEST_BUDGET } from "constant";
...
export const exportBudgetRequest = () => async (dispatch) => {
  try {
    const response = await axios({
      url: `${API_BUDGET_REQUEST}/api/BudgetRequest/ExportExcel`,
      method: "post",
      headers: getHeaders(store.getState().auth.token),
      responseType: "blob",
    });
