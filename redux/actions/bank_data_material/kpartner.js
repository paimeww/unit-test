...
import { API_SUPPLIER_MANAGEMENT, API_SUPPLIER_MANAGEMENT_URL } from "constant";
  ...
  (pageNumber, pageSize, searchQuery) => async (dispatch) => {
    try {
      ...
      const response = await axios({
        url: `${API_SUPPLIER_MANAGEMENT}/api/Catalogue`,
        method: "get",
        headers: {
          ...header,
          "X-PAGINATION": true,
...
export const getBankDataMaterialByID = (id) => async (dispatch) => {
  try {
    const response = await axios({
      url: `${API_SUPPLIER_MANAGEMENT}/api/Catalogue/${id}`,
      method: "get",
