...
const PrintoutExCA = ({ data, sessionData, userRoles }) => {
  ...
  const filterDataBuyer =
    data.length > 1
      ? data.filter((element) => element.buyerSite != "KALBEGROUP")
      : data;
  // ** PrintoutExCA on mount
  useEffect(() => window.print(), []);

  return (
    <Card className="invoice-preview-card ml-2">
      <br />
      <CardBody className="invoice-padding pb-0">
        {/* Header */}
        <center>
          <h4 className="font-weight-bolder">SUPPLY CONTRACT</h4>
          <h4 className="font-weight-bolder">KALBE GROUP</h4>
        ...
        <Row className="invoice-spacing">
          <Col className="p-0" lg="8">
            ...
            {filterDataBuyer.map((item, index) => {
              return (
                <div key={index} className="d-flex flex mt-1">
      ...
      {/* Invoice Note */}
      <CardBody className="invoice-padding pt-0 mt-2">
        <Row>
          <Col sm="6">
          ...
          <Col sm="6">
            ...
            <div
              className=""
              style={{ height: 1, backgroundColor: "black", width: 120 }}
            ></div>
          </Col>
        </Row>
      </CardBody>
