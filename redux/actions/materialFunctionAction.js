...
export const getAllMatFunc = () => async (dispatch) => {
  try {
    const header = getHeaders(store.getState().auth.token);

    const response = await axios({
      url: `${API_MASTER_DATA}/api/master/InfoForMaterialFunction`,
      method: "get",
      headers: {
        ...header,
        "X-PAGINATION": true,
...
//   try {
//     const response = await axios({
//       url: `${API_MASTER_DATA}/api/master/InfoForMaterialFunction/${id}`,
//       method: "delete",
//       headers: getHeaders(store.getState().auth.token),
//     });
